...
public class CustomeExceptionController extends ResponseEntityExceptionHandler {
	...
	@ExceptionHandler(value = EmployeeNotFoundBYIDsException.class)
	public ResponseEntity<ExceptionDetails> employeeNotFoundException(EmployeeNotFoundBYIDsException exception) {
		ExceptionDetails exceptionDetails = new ExceptionDetails(exception.getMessage(), false);
		return new ResponseEntity<ExceptionDetails>(exceptionDetails, HttpStatus.NOT_FOUND);
	}
	
	@ExceptionHandler(value = CommonExcaptionHandler.class)
	public ResponseEntity<ExceptionDetails> commonExcaptionHandler(CommonExcaptionHandler exception) {
		ExceptionDetails exceptionDetails = new ExceptionDetails(exception.getMessage(), false);
		return new ResponseEntity<ExceptionDetails>(exceptionDetails, HttpStatus.BAD_REQUEST);

	}
	
	@ExceptionHandler(value = StatusIsActiveOrNot.class)
	public ResponseEntity<ExceptionDetails> exception(StatusIsActiveOrNot exception) {
		ExceptionDetails exceptionDetails = new ExceptionDetails(exception.getMessage(), false);
		return new ResponseEntity<ExceptionDetails>(exceptionDetails, HttpStatus.BAD_REQUEST);
	}
